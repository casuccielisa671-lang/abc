...
- **资讯管理页宽度对齐修复（2026-07-16）**：`views/admin/NewsManage.vue` 原本 `<style scoped>` 里写死 `.news-manage { max-width: 1100px }` 且本地把 `.page-title` 缩到 22px、页头间距改小，导致它比其他管理页（报告中心/用户管理/采集管理等都不限宽、标题走全局 28px）**更窄、大小不兼容**。修法：**删掉这 4 行本地覆盖**（max-width + 本地 `.page-head/.page-title/.page-sub`），改用全局共享页头样式、铺满内容区。**纯 CSS，仅动这一个文件的样式块，不碰模板/逻辑/功能**。`npm run build` 通过。
