...
> ⚠️ **本文档已被取代**：深度优化后请以 **`docs/项目开发说明书.md`** 为准（架构、分工、时间线、待办清单全部更新）。
> 主要变化：A/B/C 组的 Service/Controller 框架已全部实现完毕（本文档中的大量"新建文件"任务已完成），
> 剩余工作以代码内 `TODO(阶段-组别)` 标注为准；技术栈已收敛（无 HDFS/HBase/Neo4j，XXL-Job 默认关闭）。
> 本文档保留仅作历史参考。
